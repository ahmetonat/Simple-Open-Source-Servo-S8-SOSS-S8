...
void HT1621_Init(void)//Initialize the HT1621 chip.
{
  ...
  //halt();
  Delayms(10);   //TODO: This can be shorter.
...
//void displaydata(uint8_t p) //Display the prepared number all at once.
//void HT1621_Refresh(uint8_t p, struct HT1621Values * LCD_data) 
//  void HT1621_Refresh(struct HT1621Values * LCD_data) 
  void HT1621_Refresh(HT1621Values * LCD_data) 
  //Display the prepared number all at once.
  //Set the decimal point at location p.
{
  ...
  /*  switch(p)
    {
    case 1:
      sbi(LCD_data->digit[0],7);
      break;
    case 2:
      sbi(LCD_data->digit[1],7);
      break;
    case 3:
      sbi(LCD_data->digit[2],7);
      break;
    default:break;
    }
  */
...
//void  HT1621_ValuesConstructor(struct HT1621Values * LCD_data){
...
//void HT1621_Convert(long number, struct HT1621Values * LCD_data)
...
//void HT1621_Blanking (struct HT1621Values * LCD_data){
void HT1621_Blanking (HT1621Values * LCD_data){
  ...
  for (i=4; i>0;--i){  //For i= digits '4' to '1'
    ...
    //    if (LCD_data->decimalPt==i||LCD_data->decimalPt!=4){
...
void HT1621_PortInit(void){
  ...
  //Removed dependency on the library:
  
  HT1621Port->ODR = GPIO_ODR_RESET_VALUE; /* Reset Output Data Register */
  HT1621Port->DDR = GPIO_DDR_RESET_VALUE; /* Reset Data Direction Register */
  HT1621Port->CR1 = GPIO_CR1_RESET_VALUE; /* Reset Control Register 1 */
  HT1621Port->CR2 = GPIO_CR2_RESET_VALUE; /* Reset Control Register 2 */
  HT1621Port->ODR=0; //All pins off

  //Change registers for the following pins:
  HT1621Port->DDR |=((1<<HT1621WR)|(1<<HT1621Data)|(1<<HT1621CS));
  HT1621Port->CR1 |=((1<<HT1621WR)|(1<<HT1621Data)|(1<<HT1621CS));
  HT1621Port->CR2 &=(!((1<<HT1621WR)|(1<<HT1621Data)|(1<<HT1621CS)));
}
